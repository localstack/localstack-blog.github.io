...
In the past, we suggested that users connect their application containers to the host network (`--network host`) or by making requests to `host.docker.internal:4566` when using Docker Desktop.
...
* If SSL is used, then certificate validation must be turned off:
    * LocalStack presents a certificate for a set of registered domains[^1];
...
Our aim with the networking improvements was to make the default configuration work without customisation for most users.
...
This type of configuration changes the behaviour of LocalStack networking.
We previously used the configuration variables:
...
We stopped using different ports for HTTP and HTTPS over two years ago, so the names were not accurate.
...
By moving the DNS server into LocalStack and configuring spawned AWS compute environments to use it by default, your Lambda functions, ECS containers, EC2 instances, and Kubernetes pods should already be able to access LocalStack at `localhost.localstack.cloud`.
...
[^1]: You can check the list of domains the LocalStack certificate is valid for by starting LocalStack, then running the following command: `openssl s_client -connect localhost.localstack.cloud:4566 2>/dev/null | openssl x509 -noout -text | grep DNS:`
